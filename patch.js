(function() {
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        try {
          ...
          if (sc) {
            ...
            var badges = {
              'active':   ['Active',   'rgba(22,160,133,0.12)', '#12866E'],
              'on hold':  ['On hold',  'rgba(231,76,124,0.12)', '#9A5DB3'],
              'draft':    ['Draft',    'rgba(107,114,128,0.1)', '#555555'],
              'archived': ['Archived', 'rgba(107,114,128,0.08)','#9E9E9E'],
            };
  ...
  // ── 4. Team loader — admin/recruiter Nearwork staff only ──────────────────
  window.loadTeamFromFirestore = async function() {
    ...
    try {
      ...
      var adminRoles = ['super_admin','admin','sr_recruiter','recruiter'];
      var members = snap.docs
        .map(function(d) { return Object.assign({id:d.id}, d.data()); })
        .filter(function(m) {
          var role  = String(m.role || '').toLowerCase();
          var email = String(m.email || '').toLowerCase();
          var nearworkEmail = email.endsWith('@nearwork.co');
          var staffRole = adminRoles.indexOf(role) > -1;
          return nearworkEmail && staffRole;
        })
        .sort(function(a, b) {
          var roleRank = {super_admin:0, admin:1, sr_recruiter:2, recruiter:3};
          var ar = roleRank[String(a.role || '').toLowerCase()] ?? 9;
          var br = roleRank[String(b.role || '').toLowerCase()] ?? 9;
      ...
      var rg = {super_admin:'linear-gradient(135deg,#16A085,#12866E)',admin:'linear-gradient(135deg,#12866E,#16A085)',sr_recruiter:'linear-gradient(135deg,#16A085,#12866E)',recruiter:'linear-gradient(135deg,#16A085,#12866E)'};
      ...
      grid.innerHTML = members.map(function(m) {
        ...
        var role  = String(m.role || 'recruiter').toLowerCase();
        return '<div class="team-card">'
          +'<div class="tc-top"><div class="tc-av" style="background:'+(rg[role]||'linear-gradient(135deg,#555555,#9E9E9E)')+'">'+ini+'</div>'
