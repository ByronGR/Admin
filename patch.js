...
(function() {
  ...
  if (_origOpenOrgDetail) {
    window.openOrgDetail = function(orgId, name, domain, initials, plan, status) {
      ...
      // Override the /dashboard/org/ URL with clean /orgs/slug
  ...
  var observer = new MutationObserver(function() {
    if (!document.getElementById('auth-guard')) {
      ...
      if (p.match(/^\/orgs\/[a-z0-9-]+/)) {
        ...
        if (orgData) { setTimeout(function(){ openOrgDetail.apply(null, orgData); },80); return; }
        // Unknown org — go to orgs list
        setTimeout(function(){ navTo('orgs','Organizations'); },80); return;
      }
