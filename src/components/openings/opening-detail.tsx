      ...
      <div className={`rounded-2xl border p-6 bg-white ${briefStatus === 'approved' && !sheetReady && !opening.published ? 'border-emerald-300 ring-2 ring-emerald-100' : 'border-[var(--border)]'}`}>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-xs text-[var(--light)]">
                The public, candidate-facing listing shown on jobs.nearwork.co.
                {!sheetReady && <span className="ml-1 text-amber-600">⚠ Public summary + one skill required to publish.</span>}
              </p>
            </div>
          </div>
          {opening.published && (
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div>
            ...
            <input value={sheet.city} onChange={(e) => setSheet((s) => ({ ...s, city: e.target.value }))} placeholder="Bogotá" className="w-full rounded-lg border border-[var(--border)] bg-[var(--bg)] px-3 py-2.5 text-sm outline-none focus:border-[var(--green)] focus:bg-white" />
          </div>
          <div className="sm:col-span-2">
            <label className="mb-1 block text-[10px] font-600 uppercase tracking-wider text-[var(--light)]">Benefits</label>
            <textarea value={sheet.benefits} onChange={(e) => setSheet((s) => ({ ...s, benefits: e.target.value }))} rows={2} placeholder="Health insurance, remote stipend, learning budget…" className="w-full resize-none rounded-lg border border-[var(--border)] bg-[var(--bg)] px-3 py-2.5 text-sm outline-none focus:border-[var(--green)] focus:bg-white" />
          </div>
        </div>
